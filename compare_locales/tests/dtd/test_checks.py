...
from compare_locales.parser import DTDEntity, Parser, getParser
...
"""
    ...
    def testWarning(self):
        self._test(
            b"""<!ENTITY foo "This is &not; good">
""",
            (("warning", (0, 0), "Referencing unknown entity `not`", "xmlparse"),),
        )
        # make sure we only handle translated entity references
        self._test(
            """<!ENTITY foo "This is &ƞǿŧ; good">
""".encode(),
            (("warning", (0, 0), "Referencing unknown entity `ƞǿŧ`", "xmlparse"),),
        )

    def testErrorFirstLine(self):
        self._test(
            b"""<!ENTITY foo "This is </bad> stuff">
""",
...
""",
    ...
    def testPercentEntity(self):
        self._test(
            b"""<!ENTITY formatPercent "Another 100&#037;">
""",
            tuple(),
        )
        self._test(
            b"""<!ENTITY formatPercent "Bad 100% should fail">
""",
    ...
    def testStyleWarnings(self):
        self._test(
            b"""<!ENTITY style "width:15ch">""",
            (("warning", 0, "height only in reference", "css"),),
        )
        self._test(
            b"""<!ENTITY style "width:15em;height:200px;">""",
...
"""
    ...
    def testOK(self):
        self._test(b"""<!ENTITY ent.start "Mit &brandShorterName;">""", tuple())

    def testMismatch(self):
        self._test(
            b"""<!ENTITY ent.start "Mit &brandShortName;">""",
            (
                (
                    "warning",
                    (0, 0),
    ...
    def testAcross(self):
        self._test(b"""<!ENTITY ent.end "Mit &brandShorterName;">""", tuple())
    ...
    def testAcrossWithMismatch(self):
        ...
        self._test(b"""<!ENTITY ent.end "Mit &brandShortName;">""", tuple())

    def testUnknownWithRef(self):
        self._test(
            b"""<!ENTITY ent.start "Mit &foopy;">""",
            (
                (
                    "warning",
                    (0, 0),
...
class TestAndroid(unittest.TestCase):
    ...
    apos_msg = (
        "Apostrophes in Android DTDs need escaping with \\' or "
    ...
    def getNext(self, v):
        ...
        return DTDEntity(ctx, None, None, (0, len(v)), (), (0, len(v)))
    ...
    def getDTDEntity(self, v):
        ...
        return DTDEntity(ctx, None, None, (0, len(v) + 16), (9, 12), (14, len(v) + 14))
    ...
    def test_android_dtd(self):
        ...
        l10n = self.getDTDEntity("''\"'")
        self.assertEqual(
            tuple(checker.check(ref, l10n)), (("error", 1, self.apos_msg, "android"),)
        )
        l10n = self.getDTDEntity('"\'""')
        self.assertEqual(
            tuple(checker.check(ref, l10n)), (("error", 2, self.quot_msg, "android"),)
        )

        # broken unicode escape
        l10n = self.getDTDEntity(rb"Some broken \u098 unicode")
        self.assertEqual(
            tuple(checker.check(ref, l10n)),
    ...
    def test_android_prop(self):
        f = File(
            "embedding/android/strings.properties",
            "strings.properties",
            "embedding/android",
        )
        checker = getChecker(f, extra_tests=["android-dtd"])
