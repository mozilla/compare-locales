...
class TestMockOS(Rooted, unittest.TestCase):
    ...
    def test_find(self):
        ...
        self.assertListEqual(
            list(self.node.find("jazz/one").walk()),
            [
                ("jazz/one", [], ["bit"]),
            ],
        )
...
class TestProjectPaths(Rooted, unittest.TestCase):
    def test_l10n_path(self):
        ...
        self.assertListEqual(
            list(files), [(self.path("/de/good.ftl"), None, None, set())]
        )
        ...
        self.assertTupleEqual(
            ...
            (self.path("/de/something.ftl"), None, "merging/de/something.ftl", set()),
        )
    ...
    def test_reference_path(self):
        ...
        cfg.set_locales(["de"])
        cfg.add_paths(
            {"l10n": "{l10n_base}/{locale}/*", "reference": self.path("/reference/*")}
        )
        mocks = [
            self.path(leaf)
            for leaf in [
    ...
    def test_partial_l10n(self):
        ...
        cfg.add_paths(
            ...
            {"l10n": self.path("/{locale}/minor/*"), "locales": ["de"]},
        )
        mocks = [
            self.path(leaf)
            for leaf in [
...
""",
    ...
    def test_de(self, _walk, _isfile):
        ...
        self.assertEqual(
            pontoon_config.filter(
                File(l10n_path, "de/firefox/feature.ftl", locale="de")
            ),
            "ignore",
        )
        ...
        ref_path = self.path("/en/firefox/home.ftl")
        self.assertEqual(
            pontoon_config.filter(File(l10n_path, "de/firefox/home.ftl", locale="de")),
            "ignore",
        )
        self.assertIsNone(pontoon_files.match(l10n_path))
        self.assertIsNone(pontoon_files.match(ref_path))
        ...
        self.assertEqual(
            pontoon_config.filter(
                File(l10n_path, "de/mozorg/mission.ftl", locale="de")
            ),
            "error",
        )
    ...
    def test_gd(self, _walk, _isfile):
        ...
        ref_path = self.path("/en/firefox/home.ftl")
        self.assertEqual(
            pontoon_config.filter(File(l10n_path, "gd/firefox/home.ftl", locale="gd")),
            "error",
        )
        self.assertIsNotNone(pontoon_files.match(l10n_path))
        self.assertIsNotNone(pontoon_files.match(ref_path))
    ...
    def test_it(self, _walk, _isfile):
        ...
        ref_path = self.path("/en/firefox/home.ftl")
        file = File(l10n_path, "it/firefox/home.ftl", locale="it")
...
""",
        ...
        cfg = parser.parse(
            self.path("/base.toml"), env={"l10n_base": self.path("/l10n")}
        )
        mocks = [
            self.path(leaf)
            for leaf in [
        ...
        cfg = parser.parse(
            self.path("/wildcard.toml"), env={"l10n_base": self.path("/l10n")}
        )
        mocks = [
            self.path(leaf)
            for leaf in [
