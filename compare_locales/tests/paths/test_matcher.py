...
class TestMatcher(unittest.TestCase):
    def test_matcher(self):
        ...
        self.assertEqual(one.sub(other, "foo/baz/one/qux"), "bar/baz/other/qux")
        self.assertEqual(one.sub(other, "foo/baz/one/qux/zzz"), "bar/baz/other/qux/zzz")
        ...
        self.assertEqual(
            one.sub(other, "foo/tender/bar/baz.qux"), "baz/tender/qux/baz.qux"
        )
    ...
    def test_prefix(self):
        ...
        self.assertEqual(
            Matcher("foo/{v}/bar", {"v": "expanded"}).prefix, "foo/expanded/bar"
        )
        self.assertEqual(Matcher("foo/{v}/*/bar").prefix, "foo/")
        self.assertEqual(
            Matcher("foo/{v}/*/bar", {"v": "expanded"}).prefix, "foo/expanded/"
        )
        self.assertEqual(Matcher("foo/{v}/*/bar", {"v": "{missing}"}).prefix, "foo/")
    ...
    def test_encoded_prefix(self):
        ...
        self.assertEqual(Matcher("foo/{v}/bar", encoding="utf-8").prefix, b"foo/")
        self.assertEqual(
    ...
    def test_variables(self):
        ...
        self.assertIsNone(
            Matcher("{ var }/foopy/{ var }/bears").match("one/foopy/other/bears")
        )
        self.assertDictEqual(
            Matcher("{ var }/foopy/{ var }/bears").match(
                "same_value/foopy/same_value/bears"
            ),
            {"var": "same_value"},
        )
        self.assertIsNone(
            Matcher("{ var }/foopy/bears", {"var": "other"}).match("one/foopy/bears")
        )
        self.assertDictEqual(
            Matcher("{ var }/foopy/bears", {"var": "one"}).match("one/foopy/bears"),
            {"var": "one"},
        )
        self.assertDictEqual(
            Matcher(
                "{one}/{two}/something", {"one": "some/segment", "two": "with/a/lot/of"}
            ).match("some/segment/with/a/lot/of/something"),
            {"one": "some/segment", "two": "with/a/lot/of"},
        )
        self.assertDictEqual(
            Matcher("{l}**", {"l": "foo/{locale}/"}).match("foo/it/path"),
            {
                "l": "foo/it/",
                "locale": "it",
                "s1": "path",
    ...
    def test_encoded_variables(self):
        ...
        self.assertDictEqual(
            Matcher("{path}/bar.file", encoding="utf-8").match(b"foo/bar.file"),
            {"path": "foo"},
        )
        self.assertDictEqual(
            Matcher("{l}*", {"l": "foo/{locale}/"}, encoding="utf-8").match(
                b"foo/it/path"
            ),
            {
                "l": "foo/it/",
    ...
    def test_variables_sub(self):
        ...
        self.assertEqual(
            one.sub(other, "ONE_BASE/ab-CD/special"), "OTHER_BASE/somewhere/special"
        )
        one = Matcher("{base}/{loc}/*", {"base": "ONE_BASE"}, encoding="utf-8")
        other = Matcher("{base}/somewhere/*", {"base": "OTHER_BASE"}, encoding="utf-8")
        self.assertEqual(
            one.sub(other, b"ONE_BASE/ab-CD/special"), b"OTHER_BASE/somewhere/special"
        )

    def test_copy(self):
        one = Matcher("{base}/{loc}/*", {"base": "ONE_BASE", "generic": "keep"})
        other = Matcher(one, {"base": "OTHER_BASE"})
        self.assertEqual(
            one.sub(other, "ONE_BASE/ab-CD/special"), "OTHER_BASE/ab-CD/special"
        )
        self.assertDictEqual(one.env, {"base": ["ONE_BASE"], "generic": ["keep"]})
        self.assertDictEqual(other.env, {"base": ["OTHER_BASE"], "generic": ["keep"]})

    def test_eq(self):
        self.assertEqual(Matcher("foo"), Matcher("foo"))
        self.assertNotEqual(Matcher("foo"), Matcher("bar"))
        self.assertEqual(Matcher("foo", root="/bar/"), Matcher("foo", root="/bar/"))
        self.assertNotEqual(Matcher("foo", root="/bar/"), Matcher("foo", root="/baz/"))
        self.assertNotEqual(Matcher("foo"), Matcher("foo", root="/bar/"))
        self.assertEqual(
            Matcher("foo", env={"one": "two"}), Matcher("foo", env={"one": "two"})
        )
        self.assertEqual(Matcher("foo"), Matcher("foo", env={}))
        self.assertNotEqual(
            Matcher("foo", env={"one": "two"}), Matcher("foo", env={"one": "three"})
        )
        self.assertEqual(
            Matcher("foo", env={"other": "val"}), Matcher("foo", env={"one": "two"})
        )
...
class TestAndroid(unittest.TestCase):
    ...
    def test_match(self):
        ...
        self.assertEqual(
            one.match("values-de/strings.xml"), {"android_locale": "de", "locale": "de"}
        )
        self.assertEqual(
            one.match("values-de-rDE/strings.xml"),
            {"android_locale": "de-rDE", "locale": "de-DE"},
        )
        self.assertEqual(
            one.match("values-b+sr+Latn/strings.xml"),
            {"android_locale": "b+sr+Latn", "locale": "sr-Latn"},
        )
        self.assertEqual(
            one.with_env({"locale": "de"}).match("values-de/strings.xml"),
            {"android_locale": "de", "locale": "de"},
        )
        self.assertEqual(
            one.with_env({"locale": "de-DE"}).match("values-de-rDE/strings.xml"),
            {"android_locale": "de-rDE", "locale": "de-DE"},
        )
        self.assertEqual(
            one.with_env({"locale": "sr-Latn"}).match("values-b+sr+Latn/strings.xml"),
            {"android_locale": "b+sr+Latn", "locale": "sr-Latn"},
        )

    def test_repeat(self):
        self.assertEqual(
            Matcher("{android_locale}/{android_locale}").match("b+sr+Latn/b+sr+Latn"),
            {"android_locale": "b+sr+Latn", "locale": "sr-Latn"},
        )
        self.assertEqual(
            Matcher(
                "{android_locale}/{android_locale}", env={"locale": "sr-Latn"}
    ...
    def test_mismatch(self):
        # test failed matches
        one = Matcher("values-{android_locale}/strings.xml")
        self.assertIsNone(one.with_env({"locale": "de"}).match("values-fr.xml"))
        self.assertIsNone(one.with_env({"locale": "de-DE"}).match("values-de-DE.xml"))
        self.assertIsNone(
            one.with_env({"locale": "sr-Latn"}).match("values-sr-Latn.xml")
        )
        self.assertIsNone(
            Matcher("{android_locale}/{android_locale}").match("b+sr+Latn/de-rDE")
        )

    def test_prefix(self):
        one = Matcher("values-{android_locale}/strings.xml")
        self.assertEqual(one.with_env({"locale": "de"}).prefix, "values-de/strings.xml")
        self.assertEqual(
            one.with_env({"locale": "de-DE"}).prefix, "values-de-rDE/strings.xml"
        )
        self.assertEqual(
            one.with_env({"locale": "sr-Latn"}).prefix, "values-b+sr+Latn/strings.xml"
        )
    ...
    def test_aliases(self):
        ...
        for legacy, standard in ANDROID_STANDARD_MAP.items():
            self.assertDictEqual(
                ...
                {"android_locale": legacy, "locale": standard},
            )
            self.assertEqual(
...
class TestRootedMatcher(Rooted, unittest.TestCase):
    ...
    def test_copy(self):
        ...
        self.assertIsNotNone(other.match(self.path("/different-root/some/path")))
    ...
    def test_variable(self):
        ...
        self.assertIsNotNone(one.match(self.path("/one-root/relative-dir/path")))
        other = Matcher(one, env={"var": r2})
        self.assertIsNone(other.match(self.path("/one-root/relative-dir/path")))
