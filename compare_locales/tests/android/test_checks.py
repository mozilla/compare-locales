...
class SimpleStringsTest(BaseHelper):
    ...
    def test_simple_string(self):
        self._test(ANDROID_WRAPPER % b"foo", tuple())

    def test_empty_string(self):
        self._test(ANDROID_WRAPPER % b"", tuple())

    def test_single_cdata(self):
        self._test(ANDROID_WRAPPER % b"<![CDATA[text]]>", tuple())
        self._test(ANDROID_WRAPPER % b"<![CDATA[\n  text\n  ]]>", tuple())
...
class QuotesTest(BaseHelper):
    ...
    def test_straightquotes(self):
        self._test(
            ANDROID_WRAPPER % b'""',
            (("error", 0, "Double straight quotes not allowed", "android"),),
        )
        self._test(ANDROID_WRAPPER % b'"some"', tuple())
        self._test(ANDROID_WRAPPER % b'some\\"', tuple())
        self._test(ANDROID_WRAPPER % b'some"', tuple())
        self._test(ANDROID_WRAPPER % b"some", tuple())
        self._test(
            ANDROID_WRAPPER % b'some""',
            (("error", 4, "Double straight quotes not allowed", "android"),),
        )

    def test_apostrophes(self):
        self._test(ANDROID_WRAPPER % b'''"some'apos"''', tuple())
        self._test(ANDROID_WRAPPER % b"""some\\'apos""", tuple())
        self._test(
            ANDROID_WRAPPER % b"""some'apos""",
            (("error", 4, "Apostrophe must be escaped", "android"),),
        )
...
class TranslatableTest(BaseHelper):
    ...
    refContent = (ANDROID_WRAPPER % b"plain").replace(
        b'name="foo"', b'translatable="false" name="foo"'
    )

    def test_translatable(self):
        self._test(
            ANDROID_WRAPPER % b'"some"',
            (("error", 0, "strings must be translatable", "android"),),
        )
...
class AtStringTest(BaseHelper):
    ...
    def test_translatable(self):
        self._test(
            ANDROID_WRAPPER % b'"some"',
            (("warning", 0, "strings must be translatable", "android"),),
        )
...
class PrintfSTest(BaseHelper):
    ...
    def test_match(self):
        self._test(ANDROID_WRAPPER % b'"%s"', tuple())
        self._test(ANDROID_WRAPPER % b'"%1$s"', tuple())
        self._test(ANDROID_WRAPPER % b'"$s %1$s"', tuple())
        self._test(ANDROID_WRAPPER % b'"$1$s %1$s"', tuple())
    ...
    def test_off_position(self):
        self._test(
            ...
            (
                ...
                ("warning", 0, "Formatter %1$s not found in translation", "android"),
            ),
        )
...
class PrintfDTest(BaseHelper):
    ...
    def test_match(self):
        self._test(ANDROID_WRAPPER % b'"%d"', tuple())
        self._test(ANDROID_WRAPPER % b'"%1$d"', tuple())
        self._test(ANDROID_WRAPPER % b'"$d %1$d"', tuple())
        self._test(ANDROID_WRAPPER % b'"$1$d %1$d"', tuple())
    ...
    def test_off_position(self):
        self._test(
            ...
            (
                ("error", 0, "Formatter %2$d not found in reference", "android"),
